...
coef_plot_barre_old= function(matrice_coefficient, nom_axes=c("Columns of Y","Variables","Coefficients value","SICPLN"),grad_echelle=c(-5,5)){
  ...
  plot_genus2_sicpln=ggplot2::ggplot(data=metlcoef_sic_genus, aes(x=as.factor(Var2), y=as.factor(Var1),
...
coef_genus_barre= function(matrice_coefficient, nom_axes=c("Genus","Variables","Coefficients value","SICPLN")){
  nom_axe_y=nom_axes[1]
  nom_axe_x=nom_axes[2]
  nom_gradient_couleur=nom_axes[3]
  titre=nom_axes[4]
  ...
  plot_genus2_sicpln=ggplot(data=metlcoef_sic_genus, aes(x=as.factor(Var2), y=as.factor(Var1),
    ...
    geom_tile_pattern(pattern_color = NA,
                      ...
                      pattern_key_scale_factor = 1) +
    scale_pattern_manual(values = c(Zero = "circle", Nonzero = "none"),name="") +
    scale_fill_gradient2(low = "darkred", high = "darkgreen",mid="white", midpoint = 0, limit = c(-3,3),name=nom_gradient_couleur)+
    coord_equal() +
    labs(x = nom_axe_y,y = nom_axe_x) +
    guides(pattern = guide_legend(override.aes = list(fill = "white")))+theme(plot.title = element_text(size = 15, hjust = 0.5), axis.title = element_text(size = 15,colour = "black"), legend.text = element_text(size = 15,colour = "black"),legend.title = element_text(color = "black", size = 15),
                                                                              axis.text.x = element_text(size=10,colour = "black",face = "bold"),strip.text.x = element_text(size = 15, colour = "black"),axis.text.y = element_text(size = 20 ,colour = "black",face = "bold"))
  return(plot_genus2_sicpln)
}

coef_plot_barre<-function(matrice_coefficient, nom_axes=c("Columns of Y","Variables","Coefficients value","SICZIPLN"),grad_echelle=c(-5,5)){
  grad_max=grad_echelle[2]#max(matrice_coefficient)
  grad_min=grad_echelle[1]#min(matrice_coefficient)
  nom_axe_y=nom_axes[1]
  nom_axe_x=nom_axes[2]
